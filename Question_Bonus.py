...
axes[0].set_title('LQR controller for inverted pendulum with step disturbance at t=0.5s')
#plt.savefig("Question_Bonus.png")
...
ax.set_ylim(-0.5, 1)
ax.set_aspect('equal')
ax.set_title('LQR controller for inverted pendulum with step disturbance at t=0.5s')
def update(frame):
    xc = x_frames[frame]
...
#anim.save("Question_Bonus.gif", writer=PillowWriter(fps=framesPerSec)) 
plt.show()